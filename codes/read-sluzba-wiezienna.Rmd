...
for (i in which(sapply(d, ncol) == 12)) {
  d[[i]]$X11 <- d[[i]]$X12
}

names(d) <- list.files("../data-raw/pdf-single-pages/")
...
prison_pdf_df[, year:=str_extract(plik, "^\\d{4}")]
prison_pdf_df[, month:=str_extract(plik, "\\d{2}\\.")]
...
unique(prison_all)[, .(n=sum(value)), .(ym=paste0(year,"-",month))]
